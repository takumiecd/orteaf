...
TEST_F(MpsKernelBaseManagerTest, AcquireCreatesNewKernelBase) {
  auto key = mps_kernel::KernelBaseKey::Create({{"lib1", "func1"}});

  auto lease = manager_.acquire(key);
...
TEST_F(MpsKernelBaseManagerTest, AcquireWithMultipleKernels) {
  auto key = mps_kernel::KernelBaseKey::Create(
...
TEST_F(MpsKernelBaseManagerTest, AcquireWithEmptyKernelsReturnInvalid) {
  mps_kernel::KernelBaseKey key{}; // Empty kernels vector
  auto lease = manager_.acquire(key);
  EXPECT_EQ(lease.operator->(), nullptr);
}

TEST_F(MpsKernelBaseManagerTest, MultipleKernelBases) {
  auto lease1 = manager_.acquire(
      mps_kernel::KernelBaseKey::Create({{"lib1", "func1"}}));
  auto lease2 = manager_.acquire(
      mps_kernel::KernelBaseKey::Create({{"lib2", "func2"}}));
  auto lease3 = manager_.acquire(
      mps_kernel::KernelBaseKey::Create({{"lib3", "func3"}}));
