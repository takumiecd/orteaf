...
protected:
  void SetUp() override {
    ...
    manager_.configure(manager_config, storage_manager_);
  }
...
TEST_F(TensorImplManagerTest, CreateTensorImpl) {
  ...
  auto lease = manager_.create(shape, DType::F32, Execution::Cpu);
...
TEST_F(TensorImplManagerTest, CreateMultipleTensorImpls) {
  ...
  auto lease1 = manager_.create(shape1, DType::F32, Execution::Cpu);
  auto lease2 = manager_.create(shape2, DType::F64, Execution::Cpu);
...
TEST_F(TensorImplManagerTest, CreateDifferentDTypes) {
  ...
  auto f16 = manager_.create(shape, DType::F16, Execution::Cpu);
  auto f32 = manager_.create(shape, DType::F32, Execution::Cpu);
  auto f64 = manager_.create(shape, DType::F64, Execution::Cpu);
  auto i32 = manager_.create(shape, DType::I32, Execution::Cpu);
...
TEST_F(TensorImplManagerTest, Transpose2D) {
  std::array<int64_t, 2> shape{3, 4};
  auto original = manager_.create(shape, DType::F32, Execution::Cpu);
...
TEST_F(TensorImplManagerTest, LeaseRelease) {
  ...
  auto lease2 = manager_.create(shape, DType::F32, Execution::Cpu);
  EXPECT_TRUE(lease2);
}
