...
TEST(DeviceBasic, EnumValuesAreDefined) {
  EXPECT_LT(device::toIndex(device::Device::CudaGeneric), device::kDeviceCount);
  EXPECT_LT(device::toIndex(device::Device::CudaH100Pcie80GB),
            device::kDeviceCount);
...
TEST(DeviceMetadata, ExecutionAndArchitectureMatchYaml) {
  EXPECT_EQ(device::executionOf(device::Device::CudaH100Pcie80GB),
            execution::Execution::Cuda);
  EXPECT_EQ(device::architectureOf(device::Device::CudaH100Pcie80GB),
            arch::Architecture::CudaSm90);

  EXPECT_EQ(device::executionOf(device::Device::MpsM3Max40c),
...
TEST(DeviceMetadata, SupportedOpsCoverYamlList) {
  ...
  EXPECT_NE(std::find(ops_vec.begin(), ops_vec.end(), ops::Op::Add),
            ops_vec.end());
  EXPECT_NE(std::find(ops_vec.begin(), ops_vec.end(), ops::Op::MatMul),
            ops_vec.end());
  EXPECT_NE(std::find(ops_vec.begin(), ops_vec.end(), ops::Op::SpikeThreshold),
            ops_vec.end());
}
