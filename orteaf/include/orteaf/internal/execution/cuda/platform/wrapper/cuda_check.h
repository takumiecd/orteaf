 ...
 */
#define CUDA_CHECK(expr)       ::orteaf::internal::execution::cuda::platform::wrapper::cudaCheck((expr), #expr, __FILE__, __LINE__)
/**
 * @def CUDA_CHECK_LAST()
 * @brief Validate the most recent CUDA Runtime error state.
 */
#define CUDA_CHECK_LAST()      ::orteaf::internal::execution::cuda::platform::wrapper::cudaCheckLast(__FILE__, __LINE__)
/**
 * @def CUDA_CHECK_SYNC(s)
 * @brief Synchronize a stream and validate only when `ORTEAF_DEBUG_CUDA_SYNC` is defined.
 */
#define CUDA_CHECK_SYNC(s)     ::orteaf::internal::execution::cuda::platform::wrapper::cudaCheckSync((s), __FILE__, __LINE__)
/**
 * @def CU_CHECK(expr)
 * @brief Validate a CUDA Driver API result and throw on failure.
 */
#define CU_CHECK(expr)         ::orteaf::internal::execution::cuda::platform::wrapper::cuDriverCheck((expr), #expr, __FILE__, __LINE__)
