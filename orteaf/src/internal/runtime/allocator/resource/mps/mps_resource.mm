...
public:
    ...
    static void setManager(::orteaf::internal::runtime::mps::MpsLibraryManager* mgr) { manager_ = mgr; }

    static PipelineLease acquirePipeline(DeviceHandle /*device*/,
                                         const LibraryKey& library_key,
                                         const FunctionKey& function_key) {
        ORTEAF_THROW_IF_NULL(manager_, "ResourcePrivateOps requires library manager");
...
private:
    static inline ::orteaf::internal::runtime::mps::MpsLibraryManager* manager_{nullptr};
};
}  // namespace
...
void MpsResource::initialize(const Config& config) {
    ...
    ORTEAF_THROW_IF_NULL(config.library_manager, "MpsResource requires non-null library_manager");
...
void MpsResource::initializeChunkAsFreelist(BufferView chunk, std::size_t chunk_size, std::size_t block_size) {
    ORTEAF_THROW_IF(!initialized_, InvalidState, "MpsResource::initializeChunkAsFreelist before initialize");
    ...
    if (!freelist_head_) {
        freelist_head_ = createBuffer(staging_heap_, 2 * sizeof(uint32_t), staging_usage_);
        auto* head_ptr = static_cast<uint32_t*>(getBufferContents(freelist_head_));
        ORTEAF_THROW_IF(head_ptr == nullptr, InvalidState, "freelist_head must be host-visible");
        head_ptr[0] = kInvalidIndex;
        head_ptr[1] = kInvalidIndex;
    }
    if (!freelist_out_) {
        freelist_out_ = createBuffer(staging_heap_, 2 * sizeof(uint32_t), staging_usage_);
        ORTEAF_THROW_IF(getBufferContents(freelist_out_) == nullptr, InvalidState, "freelist_out must be host-visible");
    }
    ...
    auto* command_queue = ::orteaf::internal::backend::mps::createCommandQueue(device_);
    ...
    auto* encoder = freelist_launcher_.createComputeEncoder(command_buffer, device_handle_, 0);
    ORTEAF_THROW_IF_NULL(encoder, "Failed to create compute encoder for freelist_init");

    freelist_launcher_.setBuffer(encoder, freelist_head_, 0, 0);                // head_offset
    freelist_launcher_.setBuffer(encoder, freelist_head_, sizeof(uint32_t), 1); // head_chunk_id
    freelist_launcher_.setBuffer(encoder, freelist_chunk_.raw(), freelist_chunk_.offset(), 2);
    ...
    freelist_launcher_.setBytes(encoder, &freelist_chunk_id_, sizeof(uint32_t), 6);
    uint32_t old_head_offset = kInvalidIndex;
    uint32_t old_head_chunk = kInvalidIndex;
    if (auto* head_ptr = static_cast<uint32_t*>(getBufferContents(freelist_head_))) {
    ...
    ::orteaf::internal::backend::mps::destroyCommandQueue(command_queue);
}

MpsResource::BufferView MpsResource::popFreelistNode() {
    ORTEAF_THROW_IF(!initialized_, InvalidState, "MpsResource::popFreelistNode before initialize");
    ...
    auto* command_buffer = freelist_launcher_.createCommandBuffer(command_queue);
    ORTEAF_THROW_IF_NULL(command_buffer, "Failed to create MPS command buffer");
    auto* encoder = freelist_launcher_.createComputeEncoder(command_buffer, device_handle_, 1);
    ORTEAF_THROW_IF_NULL(encoder, "Failed to create compute encoder for freelist_pop");

    freelist_launcher_.setBuffer(encoder, freelist_head_, 0, 0);                // head_offset
    freelist_launcher_.setBuffer(encoder, freelist_head_, sizeof(uint32_t), 1); // head_chunk_id
    freelist_launcher_.setBuffer(encoder, current_chunk.raw(), current_chunk.offset(), 2);
    ...
    auto* out_ptr = static_cast<uint32_t*>(getBufferContents(freelist_out_));
    ...
    ORTEAF_THROW_IF(chunk_id >= chunks_.size(), InvalidState, "Invalid chunk_id returned from freelist");
...
void MpsResource::pushFreelistNode(BufferView view) {
    ORTEAF_THROW_IF(!initialized_, InvalidState, "MpsResource::pushFreelistNode before initialize");
    if (!freelist_head_ || !view) {
        return;
    }
    auto it = chunk_lookup_.find(view.raw());
    ORTEAF_THROW_IF(it == chunk_lookup_.end(), InvalidParameter, "View must belong to a registered chunk");
    const uint32_t chunk_id = it->second;
    const BufferView chunk = chunks_[chunk_id];
    ORTEAF_THROW_IF(!chunk.contains(view, view.size()), InvalidParameter, "View must belong to freelist chunk");

    const std::size_t diff = view.offset() - chunk.offset();
    ORTEAF_THROW_IF(diff % freelist_block_size_ != 0, InvalidParameter, "View not aligned to block size");
    ...
    auto* command_buffer = freelist_launcher_.createCommandBuffer(command_queue);
    ORTEAF_THROW_IF_NULL(command_buffer, "Failed to create MPS command buffer");
    auto* encoder = freelist_launcher_.createComputeEncoder(command_buffer, device_handle_, 2);
    ORTEAF_THROW_IF_NULL(encoder, "Failed to create compute encoder for freelist_push");
...
void MpsResource::destroyFreelist() {
    if (freelist_head_) {
        destroyBuffer(freelist_head_);
        freelist_head_ = nullptr;
    }
    if (freelist_out_) {
        destroyBuffer(freelist_out_);
