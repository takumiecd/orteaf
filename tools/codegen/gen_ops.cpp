...
void ExpectKeys(const YAML::Node& node, std::string_view context, std::initializer_list<std::string_view> allowed) {
...
int ReadInt(const YAML::Node& node, std::string_view key, std::optional<int> default_value, std::string_view context) {
    const auto value = node[key];
    if (!value) {
        if (default_value) {
...
bool ReadBool(const YAML::Node& node, std::string_view key, bool default_value, std::string_view context) {
    const auto value = node[key];
    if (!value) {
        return default_value;
    }
...
std::optional<std::string> ReadOptionalScalarString(const YAML::Node& node, std::string_view key,
...
DTypeCatalog LoadDTypeCatalog(const fs::path& dtype_yaml_path) {
    ...
    for (std::size_t i = 0; i < dtypes_node.size(); ++i) {
        ...
        if (bit_index >= 64) {
            Fail("Currently only up to 64 dtypes are supported by gen_ops (bitset limitation)");
        }
    ...
    catalog.all_mask = (catalog.ids.size() == 64) ? std::numeric_limits<std::uint64_t>::max()
...
InputDef ParseInput(const YAML::Node& node, std::string_view context) {
    ...
    ExpectKeys(node, context, {"name", "description", "dtype_constraints", "optional"});
    InputDef input;
    ...
    const std::string constraint_context = std::string(context) + ".dtype_constraints";
    ExpectKeys(constraint_node, constraint_context,
               {"mode", "dtypes", "categories", "reference", "allow_promotion", "require_same_shape"});
    input.constraint.mode = ReadRequiredString(constraint_node, "mode", constraint_context);
    input.constraint.dtypes = ReadStringList(constraint_node, "dtypes", constraint_context);
    input.constraint.categories = ReadStringList(constraint_node, "categories", constraint_context);
    if (auto reference = ReadOptionalString(constraint_node, "reference", constraint_context)) {
    ...
    input.constraint.require_same_shape =
        ReadBool(constraint_node, "require_same_shape", false, constraint_context);
    return input;
}
...
AttributeDef ParseAttribute(const YAML::Node& node, std::string_view context) {
    ...
    ExpectKeys(node, context, {"name", "type", "default", "required", "description"});
...
ComputePolicyDef ParseComputePolicy(const YAML::Node& node, std::string_view context) {
    if (!node || !node.IsMap()) {
        std::ostringstream oss;
        oss << "Missing required mapping key 'compute_policy' in " << context;
        Fail(oss.str());
    }
    ExpectKeys(node, context, {"kind", "inputs", "input", "dtype", "function", "handler"});
...
ShapeInferenceDef ParseShapeInference(const YAML::Node& node, std::string_view context) {
    if (!node || !node.IsMap()) {
        std::ostringstream oss;
...
ParsedOpsFile ParseOpsFile(const fs::path& ops_yaml_path) {
    YAML::Node root;
    try {
    ...
    if (!root || !root.IsMap()) {
        std::ostringstream oss;
        oss << "Root of '" << ops_yaml_path << "' must be a mapping";
        Fail(oss.str());
    }

    ParsedOpsFile parsed;
    parsed.schema_version = ReadRequiredString(root, "schema_version", "root");
    parsed.catalogs = ParseCatalogs(root);

    const auto ops_node = root["ops"];
    if (!ops_node || !ops_node.IsSequence()) {
        Fail("Missing required sequence key 'ops'");
    }
    parsed.ops.reserve(ops_node.size());
    for (std::size_t i = 0; i < ops_node.size(); ++i) {
        ...
        ExpectKeys(node, context,
                   {"id", "display_name", "category", "arity", "inputs", "outputs", "attributes",
                    "compute_policy", "shape_inference", "metadata"});
        OpDefinition op;
        op.id = ReadRequiredString(node, "id", context);
        if (auto display_name = ReadOptionalString(node, "display_name", context)) {
        ...
        for (std::size_t j = 0; j < inputs_node.size(); ++j) {
            op.inputs.emplace_back(ParseInput(inputs_node[j], context + ".inputs[" + std::to_string(j) + "]"));
        }
        ...
        for (std::size_t j = 0; j < outputs_node.size(); ++j) {
            op.outputs.emplace_back(ParseOutput(outputs_node[j], context + ".outputs[" + std::to_string(j) + "]"));
        }
        ...
        if (attributes_node) {
            ...
            for (std::size_t j = 0; j < attributes_node.size(); ++j) {
                op.attributes.emplace_back(
                    ParseAttribute(attributes_node[j], context + ".attributes[" + std::to_string(j) + "]"));
            }
        }

        op.compute_policy = ParseComputePolicy(node["compute_policy"], context + ".compute_policy");
        op.shape_inference = ParseShapeInference(node["shape_inference"], context + ".shape_inference");
...
DTypeConstraintMode ParseConstraintMode(const std::string& mode, std::string_view context) {
    if (mode == "allow") {
...
DTypeRuleKind ParseDTypeRuleKind(const std::string& kind, std::string_view context) {
    if (kind == "promote") {
...
ComputePolicyKind ParseComputePolicyKind(const std::string& kind, std::string_view context) {
    if (kind == "same_as") {
...
std::uint32_t ResolveDTypeIndex(const std::string& dtype_id, const DTypeCatalog& catalog, std::string_view context) {
...
std::uint64_t MaskForDTypes(const std::vector<std::string>& dtypes, const DTypeCatalog& catalog,
                            std::string_view context) {
    std::uint64_t mask = 0;
...
std::uint64_t MaskForCategories(const std::vector<std::string>& categories, const DTypeCatalog& catalog,
                                std::string_view context) {
    std::uint64_t mask = 0;
    for (const auto& category : categories) {
        ...
        if (it == catalog.category_masks.end()) {
            std::ostringstream oss;
            oss << "Unknown dtype category '" << category << "' referenced in " << context;
            Fail(oss.str());
        }
        mask |= it->second;
    }
...
void ValidateAttributeDefault(const ResolvedAttribute& attribute, std::string_view context) {
    ...
    auto fail_type = [&](const std::string& message) {
        std::ostringstream oss;
        oss << "Attribute '" << attribute.name << "' default mismatch in " << context << ": " << message;
        Fail(oss.str());
    };
    if (type == "int") {
        if (!IsInteger(value)) {
...
std::vector<ResolvedOp> ResolveOps(const ParsedOpsFile& parsed, const DTypeCatalog& dtype_catalog) {
    if (parsed.schema_version != "1.0") {
        std::ostringstream oss;
        oss << "Unsupported schema_version '" << parsed.schema_version << "', expected '1.0'";
        Fail(oss.str());
    }
    ...
    for (std::size_t op_index = 0; op_index < parsed.ops.size(); ++op_index) {
        ...
        if (!parsed.catalogs.op_categories.empty() && !parsed.catalogs.op_categories.count(op.category)) {
            std::ostringstream oss;
        ...
        resolved_op.display_name = op.display_name.empty() ? op.id : op.display_name;
        ...
        for (std::size_t i = 0; i < op.inputs.size(); ++i) {
            ...
            const std::string input_context = context + ".inputs[" + std::to_string(i) + "]";
        ...
        for (std::size_t i = 0; i < op.inputs.size(); ++i) {
            ...
            resolved_input.mode = ParseConstraintMode(input.constraint.mode, input_context + ".dtype_constraints.mode");
            std::uint64_t dtype_mask = MaskForDTypes(input.constraint.dtypes, dtype_catalog,
            ...
            case DTypeConstraintMode::Allow: {
                ...
                resolved_input.reference_input = std::numeric_limits<std::uint32_t>::max();
            ...
            case DTypeConstraintMode::Deny: {
                ...
                resolved_input.allow_mask = dtype_catalog.all_mask & ~resolved_input.deny_mask;
                resolved_input.reference_input = std::numeric_limits<std::uint32_t>::max();
                resolved_input.allow_promotion = false;
                resolved_input.require_same_shape = false;
                break;
            ...
            case DTypeConstraintMode::Match: {
                if (input.constraint.reference.empty()) {
                    std::ostringstream oss;
                    oss << "Constraint mode 'match' requires 'reference' in " << input_context;
                    Fail(oss.str());
                }
                auto it = input_indices.find(input.constraint.reference);
                if (it == input_indices.end()) {
                    std::ostringstream oss;
                    oss << "Constraint mode 'match' references unknown input '" << input.constraint.reference
                        << "' in " << input_context;
                    Fail(oss.str());
                }
        ...
        for (std::size_t i = 0; i < op.outputs.size(); ++i) {
            ...
            const std::string output_context = context + ".outputs[" + std::to_string(i) + "]";
            ...
            resolved_output.kind = ParseDTypeRuleKind(output.kind, output_context + ".dtype_rule.kind");

            auto resolve_input_list = [&](const std::vector<std::string>& names, std::string_view field_context) {
                if (names.empty()) {
                    std::ostringstream oss;
                ...
                for (const auto& name : names) {
                    ...
                    if (it == input_indices.end()) {
                        std::ostringstream oss;
                        oss << "Unknown input '" << name << "' referenced in " << field_context;
                        Fail(oss.str());
                    }
                    mask |= (1ULL << it->second);
                }
            ...
            case DTypeRuleKind::Promote:
                resolved_output.input_mask =
                    resolve_input_list(output.inputs, output_context + ".dtype_rule.inputs");
                resolved_output.reference_input = std::numeric_limits<std::uint32_t>::max();
            ...
            case DTypeRuleKind::SameAs: {
                ...
                if (it == input_indices.end()) {
                    std::ostringstream oss;
                    oss << "dtype_rule 'same_as' references unknown input '" << output.input
                        << "' in " << output_context;
                    Fail(oss.str());
                }
                resolved_output.reference_input = static_cast<std::uint32_t>(it->second);
            ...
            case DTypeRuleKind::Fixed:
                ...
                resolved_output.fixed_dtype =
                    ResolveDTypeIndex(output.dtype, dtype_catalog, output_context + ".dtype_rule.dtype");
                resolved_output.input_mask = 0;
                resolved_output.reference_input = std::numeric_limits<std::uint32_t>::max();
                break;
            case DTypeRuleKind::Custom:
                if (output.function.empty()) {
                    std::ostringstream oss;
                    oss << "dtype_rule 'custom' requires 'function' in " << output_context;
                    Fail(oss.str());
                }
        ...
        for (std::size_t i = 0; i < op.attributes.size(); ++i) {
            ...
            const std::string attribute_context = context + ".attributes[" + std::to_string(i) + "]";
            if (!attribute_names.insert(attribute.name).second) {
                std::ostringstream oss;
                oss << "Duplicate attribute name '" << attribute.name << "' in " << context;
                Fail(oss.str());
            }
        ...
        resolved_op.compute_policy.kind =
            ParseComputePolicyKind(op.compute_policy.kind, context + ".compute_policy.kind");
        auto resolve_policy_inputs = [&](const std::vector<std::string>& names,
        ...
        case ComputePolicyKind::SameAs: {
            if (op.compute_policy.input.empty()) {
                std::ostringstream oss;
                oss << "compute_policy 'same_as' requires 'input' in " << context << ".compute_policy";
                Fail(oss.str());
            }
            auto it = input_indices.find(op.compute_policy.input);
            if (it == input_indices.end()) {
                std::ostringstream oss;
                oss << "compute_policy 'same_as' references unknown input '" << op.compute_policy.input << "'";
                Fail(oss.str());
            }
            resolved_op.compute_policy.reference_input = static_cast<std::uint32_t>(it->second);
            break;
        }
        case ComputePolicyKind::Promote:
            resolved_op.compute_policy.input_mask =
                resolve_policy_inputs(op.compute_policy.inputs, context + ".compute_policy.inputs");
            if (resolved_op.compute_policy.input_mask == 0) {
                std::ostringstream oss;
                oss << "compute_policy 'promote' must specify non-empty 'inputs' in " << context;
                Fail(oss.str());
            }
            break;
        case ComputePolicyKind::Fixed:
            ...
            resolved_op.compute_policy.fixed_dtype =
                ResolveDTypeIndex(op.compute_policy.dtype, dtype_catalog, std::string(context) + ".compute_policy.dtype");
            break;
        case ComputePolicyKind::DerivedComputeType:
            if (op.compute_policy.function.empty()) {
                std::ostringstream oss;
                oss << "compute_policy 'derived_compute_type' requires 'function' in " << context;
                Fail(oss.str());
            }
            resolved_op.compute_policy.function = op.compute_policy.function;
            resolved_op.compute_policy.input_mask =
                resolve_policy_inputs(op.compute_policy.inputs, std::string(context) + ".compute_policy.inputs");
            break;
        ...
        if (!parsed.catalogs.shape_kinds.empty() && !parsed.catalogs.shape_kinds.count(op.shape_inference.kind)) {
            std::ostringstream oss;
            oss << "Op '" << op.id << "' references unknown shape_inference.kind '" << op.shape_inference.kind << "'";
            Fail(oss.str());
        }
...
GeneratedData GenerateOutputs(const std::vector<ResolvedOp>& ops, const DTypeCatalog& dtype_catalog) {
    (void)dtype_catalog;
    GeneratedData generated;
    std::ostringstream def_stream;
    def_stream << "// Auto-generated. Do not edit.\n";
    for (const auto& op : ops) {
        def_stream << "ORTEAF_OP(" << op.id << ", \"" << EscapeStringLiteral(op.display_name) << "\", \""
    ...
    header_stream << "inline constexpr std::size_t kTotalInputPortCount = " << total_inputs << ";\n";
    header_stream << "inline constexpr std::size_t kTotalOutputPortCount = " << total_outputs << ";\n";
    header_stream << "inline constexpr std::size_t kTotalAttributeCount = " << total_attributes << ";\n";
    header_stream << "inline constexpr std::uint32_t kInvalidIndex = 0xFFFFFFFFu;\n\n";

    header_stream << "enum class DTypeConstraintMode : std::uint8_t { Allow, Deny, Match };\n";
    header_stream << "enum class DTypeRuleKind : std::uint8_t { Promote, SameAs, Fixed, Custom };\n";
    header_stream << "enum class ComputePolicyKind : std::uint8_t { SameAs, Promote, Fixed, DerivedComputeType, Custom };\n\n";
    ...
    header_stream << "inline constexpr std::array<std::string_view, kOpCount> kOpIds = {\n";
    for (const auto& op : ops) {
        header_stream << "    \"" << EscapeStringLiteral(op.id) << "\",\n";
    }
    header_stream << "};\n\n";

    header_stream << "inline constexpr std::array<std::string_view, kOpCount> kOpDisplayNames = {\n";
    for (const auto& op : ops) {
        header_stream << "    \"" << EscapeStringLiteral(op.display_name) << "\",\n";
    }
    header_stream << "};\n\n";

    header_stream << "inline constexpr std::array<std::string_view, kOpCount> kOpCategories = {\n";
    for (const auto& op : ops) {
        header_stream << "    \"" << EscapeStringLiteral(op.category) << "\",\n";
    }
    header_stream << "};\n\n";

    header_stream << "inline constexpr std::array<std::uint32_t, kOpCount> kOpArity = {\n";
    for (const auto& op : ops) {
        header_stream << "    " << op.arity << ",\n";
    }
    header_stream << "};\n\n";

    header_stream << "inline constexpr std::array<Range, kOpCount> kOpInputRanges = {\n";
    ...
    if (total_inputs == 0) {
        header_stream << "inline constexpr std::array<InputSpec, 0> kOpInputSpecs = {};\n\n";
    } else {
        header_stream << "inline constexpr std::array<InputSpec, kTotalInputPortCount> kOpInputSpecs = {\n";
        for (const auto& op : ops) {
            for (const auto& input : op.inputs) {
                ...
                header_stream << ", " << FormatBitMask(input.allow_mask) << ", " << FormatBitMask(input.deny_mask)
                              << ", ";
                if (input.reference_input == std::numeric_limits<std::uint32_t>::max()) {
    ...
    header_stream << "inline constexpr std::array<Range, kOpCount> kOpOutputRanges = {\n";
    ...
    if (total_outputs == 0) {
        header_stream << "inline constexpr std::array<OutputSpec, 0> kOpOutputSpecs = {};\n\n";
    } else {
        header_stream << "inline constexpr std::array<OutputSpec, kTotalOutputPortCount> kOpOutputSpecs = {\n";
        for (const auto& op : ops) {
            for (const auto& output : op.outputs) {
                header_stream << "    OutputSpec{\"" << EscapeStringLiteral(output.name) << "\", \""
                              << EscapeStringLiteral(output.description) << "\", DTypeRuleKind::";
                switch (output.kind) {
                case DTypeRuleKind::Promote:
                ...
                if (output.reference_input == std::numeric_limits<std::uint32_t>::max()) {
                    header_stream << "kInvalidIndex";
                } else {
    ...
    header_stream << "inline constexpr std::array<Range, kOpCount> kOpAttributeRanges = {\n";
    std::size_t attribute_offset = 0;
    for (const auto& op : ops) {
        header_stream << "    Range{" << attribute_offset << ", " << op.attributes.size() << "},\n";
        attribute_offset += op.attributes.size();
    }
    header_stream << "};\n\n";

    if (total_attributes == 0) {
        header_stream << "inline constexpr std::array<AttributeSpec, 0> kOpAttributes = {};\n\n";
    } else {
        header_stream << "inline constexpr std::array<AttributeSpec, kTotalAttributeCount> kOpAttributes = {\n";
        for (const auto& op : ops) {
            for (const auto& attribute : op.attributes) {
                header_stream << "    AttributeSpec{\"" << EscapeStringLiteral(attribute.name) << "\", \""
                              << EscapeStringLiteral(attribute.type) << "\", \""
                              << EscapeStringLiteral(attribute.default_value.value_or("")) << "\", "
    ...
    header_stream << "inline constexpr std::array<ComputePolicySpec, kOpCount> kOpComputePolicies = {\n";
    for (const auto& op : ops) {
        ...
        if (op.compute_policy.reference_input == std::numeric_limits<std::uint32_t>::max()) {
            header_stream << "kInvalidIndex";
        } else {
            header_stream << op.compute_policy.reference_input;
        }
        header_stream << ", ";
        if (op.compute_policy.fixed_dtype == std::numeric_limits<std::uint32_t>::max()) {
            header_stream << "kInvalidIndex";
        } else {
    ...
    header_stream << "inline constexpr std::array<ShapeInferenceSpec, kOpCount> kOpShapeInference = {\n";
    for (const auto& op : ops) {
        header_stream << "    ShapeInferenceSpec{\"" << EscapeStringLiteral(op.shape_inference.kind) << "\", \""
    ...
    header_stream << "inline constexpr std::array<MetadataSpec, kOpCount> kOpMetadata = {\n";
    std::size_t tag_offset = 0;
    std::size_t alias_offset = 0;
    for (const auto& op : ops) {
        header_stream << "    MetadataSpec{\"" << EscapeStringLiteral(op.metadata.description) << "\", "
                      << (op.metadata.commutative ? "true" : "false") << ", "
                      << (op.metadata.differentiable ? "true" : "false") << ", "
                      << "Range{" << tag_offset << ", " << op.metadata.tags.size() << "}, "
                      << "Range{" << alias_offset << ", " << op.metadata.aliases.size() << "}},\n";
        tag_offset += op.metadata.tags.size();
        alias_offset += op.metadata.aliases.size();
        metadata_tags.insert(metadata_tags.end(), op.metadata.tags.begin(), op.metadata.tags.end());
        metadata_aliases.insert(metadata_aliases.end(), op.metadata.aliases.begin(), op.metadata.aliases.end());
    }
    header_stream << "};\n\n";

    if (metadata_tags.empty()) {
        header_stream << "inline constexpr std::array<std::string_view, 0> kOpMetadataTags = {};\n\n";
    } else {
        header_stream << "inline constexpr std::array<std::string_view, " << metadata_tags.size()
                      << "> kOpMetadataTags = {\n";
    ...
    if (metadata_aliases.empty()) {
        header_stream << "inline constexpr std::array<std::string_view, 0> kOpMetadataAliases = {};\n\n";
    } else {
        header_stream << "inline constexpr std::array<std::string_view, " << metadata_aliases.size()
                      << "> kOpMetadataAliases = {\n";
...
int main(int argc, char** argv) try {
    if (argc != 3) {
        std::cerr << "Usage: gen_ops <ops.yml> <output_dir>\n";
        return 1;
    }
    ...
    fs::path dtype_yaml = input_path.parent_path().parent_path() / "dtype" / "dtypes.yml";
